...
后续所有项目推进中新增或调整 AI 功能按钮，必须沿用统一按钮规范，不允许在单页内重新写一套独立尺寸。

- 按钮基类：图标型 AI 动作用 `ai-entry ai-entry-button`，胶囊入口用 `ai-entry ai-entry-chip`，文本入口至少保持同一字号与字重。
- 标准字号：普通模式 `24rpx`，大字体模式 `26rpx`，可读模式 `32rpx`；字重统一 `700`。
- 标准图标：默认 `40rpx`，大字体模式 `44rpx`，可读模式 `48rpx`；运行态 loader 与静态图标同尺寸。
- 标准高度：常规 AI 按钮基线 `56rpx`；受页面动作区影响时可以跟随当前动作区高度，但字体、图标、间距必须保持 AI 标准。
- 标准间距：图标与文字间距 `10rpx`，按钮水平内边距 `18rpx`，圆角 `18rpx`。
- 按钮长度：可按场景自适应；同一动作区内若空间允许，优先保持相近宽度。
- 运行态：必须用智慧行星眼 loader，不再使用普通 spinner 或机器人 loader。
- 文案：保持短句，优先 `AI整理`、`AI研判`、`AI复盘`、`AI唤醒`、`AI建议下一步`、`生成中...` 这一组表达。
